...
\begin{problem}\label{P5.1} Derive Eq. \eqref{eq:56} using the approximate second derivative formulas. This is
...
\begin{problem}\label{P5.2} Derive Eq. \eqref{eq:511} from Eqs. \eqref{eq:59} and \eqref{eq:510}. Again, just use paper and pencil.
...
you of the basic animation commands:
	\marginpar{\includegraphics[width=\marginparwidth]{fig51}\captionof{figure}{Snapshots of the evolution of a wave on a string with fixed ends and an initial displacement but no initial velocity. (See Problem 5.3(b))}\label{fig:25}}
\begin{lstlisting}
...
sometimes (unfairly) just the Courant condition
	\marginpar{\includegraphics[width=\marginparwidth]{fig53}\captionof{figure}{ Snapshots of the evolution of a wave on a string with fixed ends and no initial displacement but with an initial velocity. (See Problem 5.3(f))}\label{fig:27}}
...
\end{enumerate}
\end{problem}
	\marginpar{\includegraphics[width=\marginparwidth]{fig54}\captionof{figure}{The maximum amplitude of oscillation decays exponentially for the damped wave equation. (Problem $5.4(\mathrm{c})$ )}\label{fig:28}}
